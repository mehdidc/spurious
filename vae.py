    ...
    for epoch in range(nb_epochs):
        for i, (x, _) in enumerate(dataloader):
            ...
            if use_cuda:
                x = x.cuda()
            x = Variable(x)
            h_mu, h_log_sigma, xrec = ae(x)
            recons_error, kl = ELBO(x, xrec, h_mu, h_log_sigma)
...
def ELBO(x, xr, mu, log_sigma):
    x = x.view(x.size(0), -1)
    xr = xr.view(xr.size(0), -1)
    recons_error = (((x - xr) ** 2).sum(1)).mean()
    kl = (-0.5 * torch.sum(1 + log_sigma - mu ** 2 - torch.exp(log_sigma), 1)).mean()
